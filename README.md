...
- 商品状态支持现货、预售、进货中、概念商品、售罄和秒杀；预售商品也可参与秒杀，名额不受 SKU 库存限制。
...
- 支持手动支付凭证上传、后台付款审核、订单隐私显示和用户确认收货；订单付款页同时展示付款总金额、钱包支付金额和待支付金额，后台订单也保留同样拆分，避免钱包全额支付时误读为订单金额为 0。钱包余额足够时可选择全额钱包支付并自动确认，也可选择其他付款方式不使用钱包；钱包余额不足时会组合钱包抵扣与所选付款方式，尾款仍需人工确认。
- 支付方式支持主付款二维码、PayPal 收款邮箱和口令红包兜底；未配置主二维码时，PayPal 收款邮箱会移动到付款码位置并保留凭证上传入口，口令红包开启时显示在其下方。
- 优惠码支持全局、指定商品、多商品范围、用户优惠码包、后台直接发放、客服补偿自动发放，以及钱包充值档位按多条赠券规则在确认收款后自动生成并发放。
...
- 成本录入、利润统计、销售报表、分类报表和 CSV 导出；财务菜单提供货币页，基准货币可按站点语言自动推断或手动固定，国际汇率和黄金价格由系统每日自动获取为只读快照，后台金额录入统一使用“货币/金额单位/具体金额”分栏并最终折算为基准货币；成本条目区分持续成本、一次性成本和采购触发成本，利润统计只计算已生效成本；利润概览支持用结果名、变量和运算符组合自定义利润公式。
- 秒杀支持一次性活动和周期计划；周期计划可按每天、每周、每月或每年生成具体秒杀场次，每场可配置多个参与商品、名额和秒杀价，后台在同一个“秒杀”菜单中通过页签切换活动与计划。
...
- 内容页面、公告、前台菜单、友情链接、媒体库和资源库；商品评论、页面评论、公告评论和论坛回复合并到“评论管理”入口中通过页签切换。
- 论坛版块、帖子、活动日志和版主管理。
