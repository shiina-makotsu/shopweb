...
<body id="top" class="min-h-screen overflow-x-hidden bg-fixed bg-center bg-cover text-slate-900 theme-{{ $appearance['theme_template'] }} {{ $bodyThemeClasses }}" style="background-color: var(--shop-background); background-image: var(--shop-page-background-image), var(--shop-page-gradient)">
    ...
    <main class="mx-auto w-full max-w-7xl min-w-0 px-4 py-4">
        ...
        @if($popupAnnouncement ?? null)
            <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 px-4 py-6 text-sm text-slate-900 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="announcement-popup-title">
                <div class="max-h-[90vh] w-full max-w-2xl overflow-hidden rounded-2xl border border-pink-100 bg-white shadow-2xl shadow-slate-900/25">
                    ...
                    <div class="flex flex-wrap justify-end gap-2 border-t border-slate-100 bg-slate-50/80 px-5 py-4 sm:px-6">
                        <a class="rounded-full border border-slate-300 bg-white px-4 py-2 text-xs font-medium text-slate-700 hover:bg-slate-100" href="{{ $path('announcements.show', $popupAnnouncement) }}">查看详情</a>
                        <form method="post" action="{{ $path('announcements.read', $popupAnnouncement) }}">
                            @csrf
                            <button class="rounded-full border border-pink-600 bg-pink-600 px-5 py-2 text-xs font-medium text-white shadow-sm shadow-pink-200 hover:bg-pink-700" type="submit">确认已读</button>
                        </form>
                    </div>
                </div>
            </div>
