...
<body id="top" class="min-h-screen bg-fixed bg-center bg-cover text-slate-900 theme-{{ $appearance['theme_template'] }}" style="background-color: var(--shop-background); background-image: var(--shop-page-background-image), var(--shop-page-gradient)">
    <header class="border-b border-slate-300 bg-white">
        <div class="border-b border-slate-200 bg-slate-100 text-xs text-slate-700">
            <div class="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-2 px-4 py-2">
                ...
                <nav class="flex flex-wrap items-center gap-x-4 gap-y-2">
                    ...
                    @else
                        <a class="hover:text-blue-700" href="{{ route('announcements.index') }}">公告</a>
                    @endauth
                    @auth
                        <a class="hover:text-blue-700" href="{{ route('user.center') }}">用户中心</a>
                        <form method="post" action="{{ route('logout') }}">
                            @csrf
                            <button class="hover:text-blue-700" type="submit">退出</button>
                        </form>
                    @else
                        <a class="hover:text-blue-700" href="{{ route('login') }}">登录</a>
                        <a class="hover:text-blue-700" href="{{ route('register') }}">注册</a>
                    @endauth
                    <a id="site-cart-target" class="inline-flex items-center gap-1 font-medium text-blue-800 hover:text-blue-900" href="{{ route('cart.show') }}" aria-label="购物车 {{ $cartCount }} 件">
        ...
        <div class="mx-auto grid max-w-7xl gap-4 px-4 py-4 md:grid-cols-[auto_1fr_auto] md:items-center">
            ...
            <form method="get" action="{{ route('products.index') }}" class="grid w-full min-w-0 gap-2 sm:grid-cols-[1fr_auto]">
                <input
                ...
                <a class="text-xs text-blue-700 hover:text-blue-900 sm:col-span-2" href="{{ route('search.index', request('q') ? ['q' => request('q')] : []) }}">综合搜索用户和商品</a>
            </form>

            <a href="{{ route('cart.show') }}" class="inline-flex items-center justify-center gap-2 rounded-sm border border-emerald-700 bg-emerald-700 px-3 py-2 text-sm font-medium text-white hover:bg-emerald-800 sm:px-4" aria-label="查看购物车">
        ...
        <nav class="border-t border-slate-200 bg-blue-800 text-sm font-medium text-white">
            <div class="mx-auto flex max-w-7xl flex-wrap px-4">
                ...
                @else
                    <a class="px-4 py-3 hover:bg-blue-900" href="{{ route('login') }}">用户登录</a>
                @endauth
            </div>
        </nav>
    </header>
        ...
        @if($popupAnnouncement ?? null)
            <div class="mb-4 rounded-sm border border-pink-300 bg-pink-50 px-4 py-3 text-sm text-pink-950">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <a class="rounded-sm border border-pink-700 bg-white px-3 py-2 text-xs font-medium text-pink-800 hover:bg-pink-100" href="{{ route('announcements.show', $popupAnnouncement) }}">查看公告</a>
                </div>
            </div>
        @endif
                ...
                <aside class="space-y-4">
                    <section class="rounded-sm border border-slate-300 bg-white">
                        ...
                        <nav class="divide-y divide-slate-100 text-sm">
                            <a class="block px-3 py-2 hover:bg-blue-50 hover:text-blue-800" href="{{ route('products.index') }}">全部商品</a>
                            @forelse($categories as $category)
                                <a class="block px-3 py-2 hover:bg-blue-50 hover:text-blue-800" href="{{ route('products.index', ['category' => $category->slug]) }}">{{ $category->name }}</a>
                            @empty
                    ...
                    <section class="rounded-sm border border-slate-300 bg-white">
                        ...
                        <div class="space-y-2 px-3 py-3 text-sm">
                            ...
                            <a class="inline-flex w-full justify-center rounded-sm border border-emerald-700 bg-emerald-700 px-3 py-2 font-medium text-white hover:bg-emerald-800" href="{{ route('cart.show') }}">结算购物车</a>
                        </div>
                    </section>
                            ...
                            @if($pages->isNotEmpty())
                                <nav class="divide-y divide-slate-100 text-sm">
                                    @foreach($pages as $page)
                                        <a class="block px-3 py-2 hover:bg-blue-50 hover:text-blue-800" href="{{ route('pages.show', $page) }}">{{ $page->title }}</a>
                                    @endforeach
                                </nav>
                            @endif
