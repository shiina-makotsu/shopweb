<x-layouts.app title="友情链接">
    <section class="rounded-sm border border-slate-300 bg-white">
        <div class="border-b border-slate-200 bg-slate-100 px-4 py-3">
            <h1 class="text-lg font-semibold">友情链接</h1>
            <p class="mt-1 text-xs text-slate-600">这里收录与本站有关的伙伴、社区和资源网站。</p>
        </div>
        <div class="grid gap-px bg-slate-200 sm:grid-cols-2 lg:grid-cols-3">
            @forelse($links as $link)
                <a class="block bg-white px-4 py-4 hover:bg-blue-50" href="{{ $link->url }}" target="_blank" rel="noopener noreferrer">
                    <div class="flex gap-3">
                        @if($link->image_path)
                            <img class="h-16 w-16 shrink-0 rounded-sm border border-slate-200 object-cover" src="{{ $link->imageUrl() }}" alt="{{ $link->site_name }}">
                        @else
                            <span class="flex h-16 w-16 shrink-0 items-center justify-center rounded-sm border border-slate-200 bg-slate-100 text-xl font-semibold text-slate-500">{{ mb_substr($link->site_name, 0, 1) }}</span>
                        @endif
                        <div class="min-w-0">
            ...
            @empty
                <p class="bg-white px-4 py-8 text-sm text-slate-600">暂无友情链接。</p>
            @endforelse
        </div>
