...
it('imports product sku rows and records stock movements', function (): void {
    ...
    writeCsv($path, [
        ['商品标题', '分类', '商品状态', '交付类型', 'SKU', '规格参数名', '规格', 'SKU图片', '售价(分)', '划线价(分)', '库存', '低库存阈值', 'SKU启用'],
        ['导入商品', '导入分类', 'published', 'contact_only', 'IMP-001', '红色 L 码', '颜色: 红 / 尺寸: L', 'https://cdn.example.com/import-red.jpg', '12345', '15000', '8', '2', '是'],
    ]);
    ...
    expect($variant->product->title)->toBe('导入商品')
        ...
        ->and($variant->spec_name)->toBe('红色 L 码')
    ...
    writeCsv($path, [
        ['title', 'category', 'status', 'fulfillment_type', 'sku', 'spec_name', 'specs', 'image_path', 'price_cents', 'stock'],
        ['导入商品更新', '导入分类', 'draft', 'shipping_required', 'IMP-001', '2026 版', '{"版本":"2026"}', 'products/import-2026.webp', '13000', '5'],
    ]);
    ...
    expect($updateResult['processed'])->toBe(1)
        ...
        ->and($variant->spec_name)->toBe('2026 版')
